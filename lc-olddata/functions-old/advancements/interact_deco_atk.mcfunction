...
advancement revoke @s only lc:interact_deco_atk

#define score_holder #current_time 

##identifying the pot
execute store result score #current_time lc_var run time query gametime
execute as @e[distance=..5,type=interaction,tag=deco] store result score @s time_interaction run data get entity @s attack.timestamp
...
execute at @e[distance=..5,type=interaction,tag=decoing,tag=deco,limit=1] run function lc:player/decoration/interact_atk
