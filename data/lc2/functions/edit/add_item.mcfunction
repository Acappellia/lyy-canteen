...
#define score_holder #item_id

##append to list
execute store result score #item_id lc_var if data storage lc2:data items[]
data modify storage lc2:data items append value {}
execute store result storage lc2:data items[-1].itemid int 1 run scoreboard players get #item_id lc_var

##set preize
execute if score #lc_setprize lc_var matches 0.. store result
execute store result storage lc2:data items[-1].baseprize int 1 run scoreboard players get #item_baseprize lc_var


from entity @p SelectedItem
...
execute unless score #lc_setprize lc_var matches 0.. unless data storage lc:data items[0].tag.baseprize run tellraw @p [{"color": "gray","text": "you did not specify a valid base prize for this item, setting with default value "},{"score":{"name":"#default_prize","objective":"lc_var"}}]
