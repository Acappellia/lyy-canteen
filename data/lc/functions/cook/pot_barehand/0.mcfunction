...
execute if entity @e[distance=..5,tag=add_ing,tag=on] run tellraw @s [{"text": "> 你关上深口锅的火","color": "#CCCCCC"}]
execute unless entity @e[distance=..5,tag=add_ing,tag=on] run playsound item.flintandsteel.use block @a ~ ~ ~ 1 1
execute unless entity @e[distance=..5,tag=add_ing,tag=on] run tellraw @s [{"text": "> 你点起深口锅的火","color": "white"}]
