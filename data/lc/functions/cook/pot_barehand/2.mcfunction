...
execute if entity @e[distance=..5,tag=add_ing,tag=on] run tellraw @s [{"text": "> 你关上烤架的火","color": "#CCCCCC"}]
execute unless entity @e[distance=..5,tag=add_ing,tag=on] run playsound item.flintandsteel.use block @a ~ ~ ~ 1 1
execute unless entity @e[distance=..5,tag=add_ing,tag=on] run tellraw @s [{"text": "> 你点起烤架的火","color": "white"}]
