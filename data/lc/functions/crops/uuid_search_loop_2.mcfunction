...
execute store success score #search_uuid_out lc_var run data modify storage lc:user crops[0].UUID set from entity @s UUID
data modify storage lc:user crops[0].UUID set from storage lc:var UUID_tmp
...
execute unless score #search_uuid_out lc_var matches 0 unless score #search_uuid_init lc_var matches 0 run function lc:crops/uuid_search_loop_3
