...
execute unless score #lc_setprize lc_var matches 1.. unless data storage lc:data items[0].tag.baseprize run tellraw @p [{"color": "red","text": "you did not specify a valid base prize for this item, setting with default value "},{"score":{"name":"#default_prize","objective":"lc_var"}}]
execute unless score #lc_setprize lc_var matches 1.. unless data storage lc:data items[0].tag.baseprize store result storage lc:data items[0].tag.baseprize int 1 run scoreboard players get #default_prize lc_var
