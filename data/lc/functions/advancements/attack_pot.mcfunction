...
execute as @e[distance=..5,type=interaction,tag=pot] if score @s time_interaction = #current_time lc_var if score @s pot_type matches 3 run tag @s add attack

##determine if the pot exists
execute as @e[distance=..5,type=interaction,tag=attack,tag=pot,limit=1] at @s run function lc:cook/qte/start_qte
