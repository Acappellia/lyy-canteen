##called when clicked 1st slot in farm 0

##clone to tmp storage
data modify storage lc:var farm_var set from storage lc:user farm[0].farm_page_0[{Slot:11b}]

##check buy animal
execute if data storage lc:var farm_var.tag.unlocked unless data storage lc:var farm_var.tag.have_animal run function lc:farm/act/buy_animal_0
...
##clone back
data modify storage lc:user farm[0].farm_page_0[{Slot:11b}] set from storage lc:var farm_var
