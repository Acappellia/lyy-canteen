##called when clicked 2nd slot in farm 2

##clone to tmp storage
data modify storage lc:var farm_var set from storage lc:user farm[0].farm_page_2[{Slot:12b}]

##check interact
execute if data storage lc:var farm_var.tag.can_interact run function lc:farm/act/interact_animal

##check buy animal
execute unless data storage lc:var farm_var.tag.have_animal run function lc:farm/act/buy_animal_2
...
##clone back
data modify storage lc:user farm[0].farm_page_2[{Slot:12b}] set from storage lc:var farm_var
