##called when clicked 2nd slot in farm 1

##clone to tmp storage
data modify storage lc:var farm_var set from storage lc:user farm[0].farm_page_1[{Slot:12b}]

##check buy animal
execute if data storage lc:var farm_var.tag.unlocked unless data storage lc:var farm_var.tag.have_animal run function lc:farm/act/buy_animal_1
...
##clone back
data modify storage lc:user farm[0].farm_page_1[{Slot:12b}] set from storage lc:var farm_var
