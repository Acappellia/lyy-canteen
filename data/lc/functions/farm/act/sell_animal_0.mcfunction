...
scoreboard players operation #get_lcid lc_var = #farm_pork_id lc_var
...
data modify storage lc:var farm_var.tag set value {menu_ui:1,unlocked:1,unlock_cost:100,growth:0,quality:75,display:{Name:'{"text":"空的栏位","color":"green","italic":false}',Lore:['{"text":"点击为本栏位添加小猪","color":"dark_green","italic":false}','[{"text":"花费： ","color":"white","italic":false},{"text":"25","color":"yellow","italic":false}]']}}
data modify storage lc:var farm_var.tag.farm_slot set from storage lc:var farm_slot_tmp
data modify storage lc:var farm_var.id set value "minecraft:birch_fence"